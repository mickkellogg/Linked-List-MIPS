...
#Notes: I don't think we need strcmp at all in the main function.
...
insert: 
    ...
    move    $t1, $a1
    lw      $t2, 0($t0)         #get the byte at address in $a0
    lw	    $t6, 4($s0)		#check for NULL pointer pt 1
    #beq     $s0, $zero, new     #if list is empty, put it at the front
    beq     $t6, $zero, new     #if list is empty, put it at the front
   
    #lw      $t4, 4($t2)         #get the next 4 bits associated with pointer to the next node
   # lw      $t4, 4($t0)
   # sw      $t0, 0($t7)         # Save pointer to previous head in first half of new node
...
new: 
    ...
    sw      $a1, 0($t7)		# store address of string in first half of node
   
    #lw      $t4, 4($t0)
    sw      $t0, 4($t7)         # Save pointer to previous head in first half of new node
